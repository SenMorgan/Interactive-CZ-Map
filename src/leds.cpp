...
struct LedState
{
    ...
    uint32_t startTime = 0;    // TimeF when the effect started
};
 ...
 * @param fadeCycles The number of times the fade effect should repeat [1, MAX_FADE_REPEATS]. Use -1 for infinite.
...
void setLed(uint8_t index, uint8_t brightness, uint16_t fadeDuration, int16_t fadeCycles, CRGB color)
{
    ...
    // Validate and set the fade count. This parameter starts the effect if it is greater than 0 or -1 for infinite.
    if ((fadeCycles > 0 && fadeCycles <= MAX_FADE_REPEATS) || fadeCycles == LOOP_INDEFINITELY)
        state.fadeCycles = fadeCycles;
    else
        Serial.printf("Fade cycles [%d] is out of bounds [1, %d] or not -1 for infinite\n", fadeCycles, MAX_FADE_REPEATS);
 ...
 * If the effect has completed for the current cycle, it turns off the LED and decreases the repeat count unless it's set to infinite.
 * If the LED is not active but has remaining repeats or is set to infinite, it restarts the effect.
    ...
    for (uint8_t i = 0; i < LEDS_COUNT; i++)
    {
        ...
        if (state.isFading)
        {
            ...
            if (elapsed < state.fadeDuration)
            {
                ...
                uint8_t currentBrightness = (uint8_t)(state.brightness * (1.0 - fadeProgress));
        ...
        {
            // Start or restart the fade effect if fadeCycles > 0 or -1 for infinite
            state.startTime = currentTime;
            state.isFading = true;
            leds[i] = state.color;
            leds[i].nscale8_video(state.brightness);
...
void ledsTask(void *pvParameters)
{
    ...
    // Start infinite fade effect on one LED to indicate the task is running
    circleLedEffect(CRGB::Green);
